...
import { getBotRoot, getBotSubPath } from '../utils/aiplayers_paths.js';
...
export class Prompter {
    constructor(agent, profile) {
        ...
        if (this.profile.embedding === false || this.profile.embedding === null) {
            embedding_model_profile = null;
        }
        else if (this.profile.embedding) {
            try {
        ...
        mkdirSync(getBotRoot(name), { recursive: true });
        writeFileSync(getBotSubPath(name, 'last_profile.json'), JSON.stringify(this.profile, null, 4), (err) => {
            if (err) {
    ...
    async _saveToFile(logFile, logEntry) {
        ...
        let logDir;
        if (task_id == null) {
            logDir = getBotSubPath(this.agent.name, 'logs');
        } else {
            logDir = getBotSubPath(this.agent.name, 'logs', `${task_id}`);
        }

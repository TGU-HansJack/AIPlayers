...
export class Agent {
    ...
    async _setupEventHandlers(save_data, init_message) {
        ...
        const respondFunc = async (username, message) => {
            ...
            const isAdminSource = username === 'ADMIN' || username === '__AIPLAYERS__';
            if (settings.only_chat_with.length > 0 && !settings.only_chat_with.includes(username) && !isAdminSource) return;
            try {
    ...
    async openChat(message) {
        ...
        message = `[${this.name}] ${message.trim()}`.trim();

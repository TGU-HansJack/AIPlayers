...
当前仓库已新增 **内嵌 mindcraft sidecar** 运行方式：

- 内嵌目录：`embedded/mindcraft`
- 本地控制面板：默认 `http://localhost:8080`
- 新命令入口：`/aiplayers spawn|list|status|send|stop|remove|panel|voice ...`
- 聊天路由：`@bot名字 你的内容`
- 游戏内面板：按 `H`
- 语音录音键：按 `V`
...
1. 进入 `embedded/mindcraft`
2. 安装依赖：`pnpm install` 或 `npm install`
3. 确保 `node_modules` 已存在
4. 启动本地专用服，或先把单人世界开放为 LAN

### 关键配置文件

- `config/aiplayers-mindcraft.json`
  - 控制 sidecar 开关、`node` 路径、mindserver 端口、默认 profile 与数据目录
- `config/aiplayers-voice.json`
  - 控制 STT/TTS、默认目标 bot、自动播报与超时

### 语音说明

- `V` 键录音结束后，会把识别文本自动发送到当前选中的真实 bot
- 如果当前没有选中 bot，会退回到 `config/aiplayers-voice.json` 的 `defaultTarget`
- `bot` 回复统一以 `[botName] ...` 形式回到聊天栏，便于现有 TTS 自动播报链路直接复用

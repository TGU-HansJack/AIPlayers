...
- `[已实现]` AI 玩家实体、服务器同步、基础装备栏、背包持久化、多个 AI 共存
- `[原型中]` 玩家风格渲染、模式切换、背包明细查看、基础工具切换
- `[规划中]` 真实玩家皮肤同步、36 格完整背包、自动整理、自动换更强装备、更多物品使用

### 生存行为系统

- `[已实现]` 自动恢复、基础战斗、砍树、挖矿、农作、简易避难所建造
- `[已实现]` 水域脱困、导航卡住恢复、快速跟随、浅层遮挡资源处理
- `[规划中]` 真实饥饿值、睡眠、自动烹饪、火把照明、药水 / 盾牌 / 远程武器使用
